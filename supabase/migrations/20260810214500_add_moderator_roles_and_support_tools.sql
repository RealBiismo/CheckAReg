-- CHECK A REG staff roles: Owner > Admin > Moderator > User.
...
begin
  select lower(email) into v_email from auth.users where id=v_id;
  if v_id is null or v_email<>'cybzerohq@gmail.com' then raise insufficient_privilege using message='Only the CHECK A REG owner can manage moderators.'; end if;
  ...
  if v_target is null then raise no_data_found using message='No verified CHECK A REG account was found for that email.'; end if;
...
begin
  select lower(email) into v_email from auth.users where id=v_id;
  if v_id is null or v_email<>'cybzerohq@gmail.com' then raise insufficient_privilege using message='Only the CHECK A REG owner can view staff management.'; end if;
